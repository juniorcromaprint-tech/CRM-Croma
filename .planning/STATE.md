## Ciclo autonomo #55 - 2026-05-30 05:08 BRT - VERDE corrigir+validar (lane Orcamento/preco DB-006b): shadow guard de preco-zero-sem-modelo APLICADO em proposta_itens + pedido_itens (warn-only, padrao #51) com prova RUNTIME 2-casos (positivo delta=1 / controle negativo delta=0, rolled-back) + achado NOVO do agent: template path do editor manual bypassa o guard C-01 e grava valor_unitario=0 (PRICE-004)

**Tipo**: corrigir (1 migration DDL: 1 fn + 2 triggers) + validar (3 provas) + health. NOW 05:08 BRT; #54 as 04:35 (~33min, sem gatilho passivo). Health VERDE: Vercel 200; API 60min 100% 200 (fn_claim ~1/min); edge mcp-bridge-worker v9 60min 100% 200 (0.3-2.9s) ZERO 5xx; Edges ACTIVE (whatsapp-webhook v46/ai-gerar-orcamento v29/portal-upload-assinatura v1/agent-cron-loop v28); branch=main HEAD e66137f=#54; guardrail HOST LIMPO (STATE 3708L/ledger 1035L/log 1895L tails coerentes, 2 untracked herdados, bash NAO consultado). 1 agent adversarial read-only (~48k) no motor Mubisys.

**Decisao (sem A/B)**: PRICE-001/002/003 + DATA-004 (lane preco do #54) = BLOCKED-Junior ou mudam preco cotado (decisao do Junior). DB-006b era o UNICO default-exec seguro da lane Orcamento (transforma landmine latente em sinal visivel SEM mudar preco). Financeiro (rotacao Sabado) auditado 5 ciclos #50-53 -> NAO repetir (#54). Encaixa a Edge pricing-engine do dia.

**Mapeamento (agent, file:line)**: (1) ai-gerar-orcamento v29: produto SEM produto_modelos -> item PULADO (itensSemMatch), nada gravado (consistente com 53 produtos uso=0); modelo existe mas BOM vazia -> item PERSISTIDO com valor_unitario sobre custoMP=0 = preco ~zero silencioso (L523-543, coluna valor_unitario confirmada). (2) editor manual: fluxo normal BLOQUEIA via guard C-01 (preco<=0); MAS template path ~L690 OrcamentoEditorPage.tsx BYPASSA C-01 e insere valor_unitario=0/valor_total=0 direto = furo NOVO (PRICE-004, agent-reported, NAO-VALIDADO por mim).

**Migration shadow_guard_orcamento_item_preco_zero_sem_modelo_cycle55 (idempotente, validada vs schema real)**: fn_orcamento_item_preco_zero_shadow() SECURITY DEFINER search_path=public,pg_temp; AFTER INSERT OR UPDATE OF valor_unitario,produto_id em proposta_itens + pedido_itens; WHEN (valor_unitario<=0 AND produto_id NOT NULL); BEGIN/EXCEPTION WHEN OTHERS THEN RETURN NEW = jamais aborta gravacao. Loga system_events event_type=orcamento_item_preco_zero_sem_modelo SO quando produto NAO tem produto_modelos ativo com BOM(modelo_materiais) nem preco_fixo>0 (= landmine DB-006/DB-007 materializado; exclui zero legitimo em produto bem-modelado).

**Validacao (3 provas)**: (1) schema validado antes (produto_modelos.ativo/preco_fixo, modelo_materiais.modelo_id, system_events shape). (2) RUNTIME smoketest rolled-back: POSITIVO produto-modelless+preco0 delta=+1; CONTROLE NEGATIVO produto-com-modelo+preco0 delta=0 (sem falso-positivo). (3) catalogo: fn SECURITY DEFINER + search_path=public,pg_temp, 2 triggers (proposta_itens+pedido_itens); pollution_check=0 (rollback limpo). DB-013 shadow (#51): 0 anomalias em ~3.5h (interim limpo).

**Mudancas prod**: 1 migration DDL (1 fn + 2 triggers, reversivel via DROP). 0 deploy Edge, 0 prod-data write (smoketest rolled-back, pollution=0). 53 produtos sem modelo seguem uso=0.
...
**Decisao (sem A/B)**: backlog P0 ESGOTADO (DB-012 refutado #50, DB-013 shadow #51, DB-006 fechado #52) -> rules v5.2 manda consumir lane P1 pelo proximo default-exec = INT-006. Validei SEM deploy: Edge cliente 1294L (>800L = Claude Code) + pre-req secret nao confirmado; deploy fail-closed cego 403-aria 100% do inbound WhatsApp. Encaixei rotacao Sabado/Financeiro com angulo NOVO (revenue leakage, nunca rodado #41-52) em vez de re-reportar "CP estavel".

**INT-006 VALIDADO (P1 lane, era [NAO-VALIDADO])**: validateSignature (whatsapp-webhook v46, L372-395) tem fail-open L379-381: `if(!appSecret){console.warn(...);return true}` -> aceita QUALQUER payload sem assinatura quando WHATSAPP_APP_SECRET ausente. Backlog dizia "L380" = off-by-one (return true real = L381; cond L379; warn L380). Deploy==fonte (agent + get_edge_function). appSecret = Deno.env WHATSAPP_APP_SECRET com fallback admin_config. DE-RISK ao vivo: admin_config tem 13 chaves WHATSAPP_* (ACCESS_TOKEN/APP_ID/VERIFY_TOKEN/PHONE_NUMBER_ID/WABA_ID/BUSINESS_ACCOUNT_ID/templates...) mas NAO tem WHATSAPP_APP_SECRET -> secret provavelmente NAO setado (toda outra cred WhatsApp mora la) -> fail-open provavelmente ATIVO em prod. Fix = L381 `return true`->`return false` (fail-closed); GET hub.challenge (L887-906) NAO usa validateSignature (handshake intacto); Meta legitimo sempre envia x-hub-signature-256. PRE-REQUISITO DURO: setar/confirmar WHATSAPP_APP_SECRET no Edge env ANTES do deploy senao 403 em 100% do inbound. 1294L -> Claude Code handoff. NAO smoketest (POST sem assinatura criaria lead/poluicao).

**Rotacao Financeiro (angulo NOVO - revenue leakage / cobertura de CR)**: 0 LEAKAGE provado. pedidos sem CR (excluido_em IS NULL): aguardando_aprovacao R$630 skip=false (CR so em aprovado = correto); aprovado R$1085.73 + concluido x3 R$3359.98 = skip_auto_cr=TRUE (mubisys by-design, OS RAVENA Beira Rio); faturado x2 R$242.34 skip=false sem_cr=0 (TEM CR). Invariante confirmada: todo pedido nao-mubisys em estado faturavel tem CR. Integridade: 0 saldo_drift em CR(2)/CP(3)/comissoes(0); saldo = valor_original-valor_pago exato. CP R$1.322,32 vencidas (3 titulos) estavel vs #51/#52 = decisao Junior pagar.

**SEC-011 reality-check (P1 backlog)**: pg_net (public, 28 deps) + pg_trgm (public, 47 deps) - mover quebraria muitos objetos (GIN trgm idx + net.http_post de crons/triggers); extension_in_public = INFO-level. DEFER (nao-seguro autonomo).

**Mudancas prod**: NENHUMA. 0 deploy / 0 migration / 0 prod-write. Commit planning #53 (cerebros).

**NEXT**: ver ledger #53 (NEXT consolidado).

---
## Ciclo autonomo #52 - 2026-05-30 02:14 BRT - VERDE corrigir(backlog P0)+validar (DB-006 + rotacao Sabado/Financeiro): relatorio dos 53 produtos ativos sem produto_modelos GERADO (P0 backlog herdado 3 ciclos, FECHADO porcao loop) + premissa de urgencia REFUTADA (todos 53 com 0 uso = landmine dormente) + Financeiro estavel

DB-006 (P0 backlog, herdado #49/#50/#51 sem acao): quantificado ao vivo -> 107 produtos, 88 ativos, 53 ativos SEM produto_modelos (60,2% dos ativos), 7 inativos sem modelo (53+7=60 reconcilia o "60" historico). Verify-before-assume: os 53 ativos sem modelo tem 0 uso em proposta_itens+pedido_itens (criados 2026-03-11, catalogo seed) -> Mubisys NUNCA custeou errado orcamento real; risco LATENTE (1o uso), nao ativo. Relatorio docs/qa-reports/2026-05-30-DB-006-produtos-sem-modelo-sem-uso (94L): numeros + breakdown por categoria (displays 11/placas 9/estruturas 8/grafica 8/fachadas 7/banners_lonas 4/adesivos 2/luminosos 2/servicos 2) + lista dos 53 + plano. Backlog [x]. Cadastro BOM = Junior (BLOCKED-Junior). NEXT proposto DB-006b: guard Mubisys/ai-gerar-orcamento p/ alertar produto-sem-modelo em vez de preco zero silencioso [NAO-VALIDADO].

Rotacao Sabado/Financeiro (read-only): CP 3 titulos a_pagar R$1.322,32 TODOS vencidos (mais antigo 2026-04-29=31d), SEM crescimento vs #51; CR 2 titulos pago saldo R$0 0 vencido. Estruturalmente saudavel, 0 anomalia nova. CP = decisao Junior.

Health VERDE: Vercel 200; edge+API 60min ZERO 5xx/4xx (mcp-bridge-worker v9, agent-cron-loop v28, fn_claim ~1/min 200); branch=main HEAD 3fb8a2d=#51; guardrail HOST LIMPO. 0 agents (data-analysis inline). 0 deploy/0 migration/0 prod-write. Commit cerebros #52 + report + backlog pushado pelo HOST. Telegram mid=3172.
...
**DB-012 (P0 correcao de dinheiro) REFUTADO**: backlog dizia "5/23 proposta_itens + 5/12 pedido_itens sem dims -> preco/m2 errado/zero". Verificado: largura_cm/altura_cm/area_m2 numeric NAO-generated; trigger fn_calc_area_m2 BEFORE INSERT OR UPDATE OF largura/altura (recalc em edit). 0 itens com area NULL E ambas dims presentes (os 10 NULL-area tem dims NULL); 0 mismatch entre area gravado e w*h. Os 10 = dimensionless legitimo (servicos Instalacao/Deslocamento/Frete mubisys, 1 teste, adesivos preco-fixo) - todos valor_total correto. Premissa FALSA, 0 backfill. Fechado sem acao.

**DB-013 (P0 state/dinheiro) MAPEADO (agent adversarial)**: fn_validar_transicao_status valida so pedidos+propostas; ordens_producao -> ELSE NULL (gap confirmado). Premissa corrigida: status tem CHECK ordens_producao_status_check (8 valores), NAO text livre. CASE canonico estava na migration 002:703-735, perdido numa reescrita = a regressao. CATCH CRITICO: auditoria real tem aguardando_programacao->em_conferencia (3x) que o mapa canonico estrito REJEITARIA -> aplicar validator estrito HOJE QUEBRARIA fluxo legitimo de prod (PCP/WhatsApp pula etapas). Bug latente: DEFAULT pendente fora do CHECK.

**Mudancas prod**: NENHUMA (read-only puro). 0 deploy/0 migration/0 prod-write. Migration SHADOW [VALIDADO] staged no NEXT (trigger ISOLADO warn-only, NAO toca o validator money-critical). #49 Memory Layer confirmado saudavel (ai_memory=8, padroes calculo upd 02:08). Commit planning #50.
...
**Decisao (sem A/B)**: perf advisors baixo-risco exauridos #42-46; Sexta=Instalacao re-auditada #47. Pivot pro P1 watch SERVICE_ROLE_KEY=JWT (portal-upload-assinatura:53), carregado SEM verificacao desde #44 (4 ciclos) = potencial bug latente. Janela aberta tornou o smoketest viavel.

**Verificacao (verify-before-assume)**: (1) get_service_role_legacy_jwt EXISTE + retorna JWT valido (left3=eyJ, len>100; boolean-only, sem leak de secret), SECURITY DEFINER, search_path=vault,public. (2) Privilegios LIVE: anon_exec=FALSE (SEC-002 revogou), authenticated_exec=true, service_role_exec=true -> SE o fallback disparar, bootstrap-anon->rpc da permission denied -> getLegacyJwtClient throw -> catch re-chama -> throw nao-capturado -> 500. (3) Source 173 LOC: path PRIMARIO usa SERVICE_ROLE_KEY direto + probe select propostas; fallback so dispara se probe der erro JWT. (4) propostas total_assinadas=0 lifetime (feature v1 NUNCA usada -> quebra seria latente no 1o uso, sem log historico).

**Smoketest (janela aberta, token invalido = ZERO write)**: POST portal-upload-assinatura {token uuid-zero, base64 min} -> HTTP 401 {ok:false,error:token invalido}. 401 (nao 500) PROVA que o probe do service client teve sucesso (chegou ao lookup de share_token e retornou null) -> SERVICE_ROLE_KEY E JWT -> fallback anon NUNCA alcancado. Bonus: funcao viva/saudavel na camada auth end-to-end.

**Veredito**: watch RESOLVIDO. SEC-002 (revoke anon EXECUTE de get_service_role_legacy_jwt) NAO quebra portal-upload-assinatura. Risco CONDICIONAL (NEXT, nao-bug-agora): o fallback getLegacyJwtClient e dead-code que daria 500 SE SERVICE_ROLE_KEY rotacionar pra sb_secret_* (o comentario-cabecalho ESPERA sb_secret_*); fix seguro exige env/code, NAO re-grantar anon (re-abriria SEC-002 + expoe service JWT ao anon). Decisao Junior.

**Mudancas prod**: NENHUMA (read-only + smoketest invalido). 0 deploy / 0 migration / 0 prod-write. Commit planning #48 (cerebros). Telegram enviado.

---
## Ciclo autonomo #47 - 2026-05-29 21:15 BRT - VERDE explorar/validar (rotacao Sexta/Instalacao, read-only): 4 watches estagnados reconciliados no RUNTIME - BUG-MCP-01 REFUTADO (v9 ja tem .select().single()), chain Producao->Instalacao DESTRAVADA confirmada (installation_order_auto_created 05-28), jobs Pendente = backlog congelado de abril (nao empilha), installation_completed re-tipificado (gargalo fisico/PWA, nao chain)

**Tipo**: explorar/validar (read-only, ZERO prod-write). NOW 21:15 BRT (00:15 UTC 30/05); #46 as 20:11 (~53min, sem gatilho passivo). Health VERDE: Vercel 200; edge 60min ZERO 5xx (mcp-bridge-worker v9 ~1/min 200, agent-cron-loop v28 200 7.7-12s); API tick natural 00:00 UTC limpo (fn_claim 200, system_events 201, lead_quente idempotency-GET 200 deduped SEM flood - v28 estavel 10o ciclo); branch=main HEAD 267cfb1=#46; guardrail HOST LIMPO (tails STATE 3593/ledger 884/log 1729, 2 untracked herdados, bash NAO consultado). 1 agent read-only (~42k, 4 tools) + 1 query SQL agregada inline.

**Decisao (sem A/B)**: #42-46 derivaram pra perf/sec advisor DDL e exauriram os de baixo risco (restantes: multiple_permissive 392 = semantica arriscada; search_path INVOKER = baixo valor). Rotacao Sexta=Instalacao sem re-audit desde #27-34; watches install/prod/jobs/BUG-MCP-01 vinham sendo copiados ha ~20 ciclos SEM reverificacao pos-Fase 1.2. Pivot pra re-audit read-only (verify-before-assume) - corrige o registro estagnado, custo zero/risco zero.

**Achados (4 watches reconciliados com evidencia de runtime)**:
- BUG-MCP-01 REFUTADO: agent leu source DEPLOYADO mcp-bridge-worker v9 (header "v8 - BUG MCP-01: insert ai_responses agora encadeia .select().single()") - insert ai_responses L86-95 JA tem .select().single() + checa insErr/!respData L96. STATE #27 ("ai_responses.insert L84 sem .select().single()") desatualizado vs deploy. Non-conformance MENOR (nao-bloqueante): 4 updates ai_requests (L59/77/98/105) sem .select() - status de fila best-effort, capturam+logam erro; arquivo 261 LOC >250 -> deploy exige agent/Claude Code, baixo valor.
- Chain Producao->Instalacao DESTRAVADA confirmada no RUNTIME: installation_order_auto_created 4x lifetime, ULTIMO 2026-05-28 17:04 UTC; 1 ordens_instalacao nova (aguardando_agendamento 05-28); 3 OPs finalizado (05-28 17:11); 1 job Em andamento (05-28); prod_transition_error=0 lifetime (sem EXCEPTION engolida); pedidos 1070/PED-2026-0025 agora concluido. Fase 1.2 (4195dc7) funciona end-to-end ATE criar ordem de instalacao.
- installation_completed STILL morto 24d (4 lifetime nesse event_type, ULTIMO 2026-05-05 02:54 UTC): RE-TIPIFICADO - NAO e bug de chain (a auto-criacao da ordem funciona). Gargalo = CONCLUSAO fisica (App Campo offline-first label, INSTAL-02 PWA) que nao roda. INSTAL-01 aberto mas causa-raiz = PWA/fisico, nao trigger.
- jobs Pendente 18 = backlog CONGELADO de abril (oldest 2026-04-01, newest 2026-04-15), 0 novos desde 04-15 -> NAO "empilhando" (refuta #27 "15 Pendente empilhando"). Stuck-estatico, nao pile-up ativo.
- MICRO: 1 ordens_instalacao "agendada" travada desde 2026-04-06 (7+ semanas sem progredir p/ concluida).

**Mudancas prod**: NENHUMA (read-only puro). 0 deploy / 0 migration / 0 prod-write. Commit planning #47 (cerebros). Telegram enviado.

---
## Ciclo autonomo #46 - 2026-05-29 20:11 BRT - VERDE arrumar (PERF/RLS): auth_rls_initplan RESOLVIDO - 78 policies public com (select auth.uid()/role()) wrap (advisor antipattern zerado), helpers get_user_role intactos, zero regressao runtime

**Tipo**: arrumar (perf/infra DDL, 1 migration) + validar. NOW 20:06->20:11 BRT (23:11 UTC); #45 as 19:20 (~46min, sem gatilho passivo). Health VERDE: Vercel 200; edge 60min ZERO 5xx (mcp-bridge-worker v9 ~1/min 200, agent-cron-loop v28 200 ~9-12s); API tick 23:00 UTC limpo ZERO 400/5xx (fn_claim 200, system_events 201, lead_quente idempotency-GET 200 deduped sem flood - v28 estavel 9o ciclo); branch=main HEAD caa2950=#45; guardrail HOST LIMPO (tails STATE 3578/ledger 856/log 1711, 2 untracked herdados, bash NAO consultado).

**Auto-dialogo (7)**: (1) #43 indexou 8 FKs, #44 validou 3 fixes seg do Junior, #45 lead_quente runtime + SEC-004 search_path 7 fns. (2) Sexta=Instalacao (mcp-bridge-worker v9), exausta #27-34 -> pivot perf advisor. (3) gap util AGORA: auth_rls_initplan (78, P1 default-exec carregado desde #41, janela idle-Junior); cross-validei meu detector = 78 public = advisor EXATO; so auth.uid+auth.role, sem current_setting. (4) sem conflito: SEC-001/buckets/views/token=BLOCKED-Junior; este e default-exec perf. (5) Obsidian: protocolo Mubisys + secret-leaks 05-27 rastreados, sem blocker novo. (6) nao-passivo (#45 ha 46min, branch main, host limpo, zero 5xx). (7) criterio: needs_fix public 78->0, zero double-wrap, zero still-bare, Vercel 200 pos-apply.

**TAREFA - auth_rls_initplan wrap (perf, advisor antipattern desde baseline #41)**: o advisor flaggeia auth.uid()/auth.role() chamadas POR-LINHA em policies RLS; fix Supabase-recomendado = embrulhar em (select ...) p/ avaliar 1x por query (initplan) ao inves de 1x por linha. Mesma semantica (auth.* sao STABLE). Reconciliacao adversarial: meu detector (strip do wrapped -> checa se bare auth.uid/role remanesce) = 84 total no banco; 78 em PUBLIC (= advisor EXATO); 6 em cron/pessoal/storage (system + app pessoal de Junior = FORA de escopo ERP, NAO tocados). distinct_fns = SO auth.uid + auth.role. 37/78 tem subquery (has_select; majoritariamente EXISTS(SELECT...), NAO wrap previo) -> usei unwrap-then-wrap p/ evitar double. 33/78 tem WITH CHECK. Preview read-only dos 78: n=78, double_wrap=0, still_bare=0, qual_unchanged=0; 8 samples eyeballados: auth.uid()->(select auth.uid()) em USING+WITH CHECK; get_user_role()/is_admin() helpers INTACTOS (advisor nao flagga; wrap deles seria escopo/risco maior); EXISTS subqueries preservadas; INSERT (USING null) e arrays USING!=WITH CHECK divergentes OK. Migration optimize_rls_initplan_auth_wrap_cycle46 (DO-block idempotente - WHERE filtra ja-wrapped, re-run=no-op; lock_timeout 5s): ALTER POLICY nos 78. success=true.

**Validacao (runtime/catalogo, nao so success)**: pos-apply still_needs_fix 78->0; double_wrapped=0; now_wrapped=78; live catalog confirma wrap (admin_config qual: profiles.id=( SELECT auth.uid() AS uid); ai_requests_insert with_check: ( SELECT auth.uid() AS uid)=solicitante_id). Vercel 200 pos-apply (ERP authenticated exercita estas policies em toda query = zero regressao). DDL-accept das 78 ALTER POLICY prova as expr todas validas/tipadas.

**Mudancas prod**: 1 migration DDL metadata (78 ALTER POLICY, reversivel, sem mudanca de tabela). 0 deploy Edge, 0 prod-data write. Commit planning #46 + migration.

**NEXT**: function_search_path_mutable ~58 SECURITY INVOKER (baixo risco, NAO-VALIDADO); multiple_permissive_policies (392, consolidar redundantes - maior/cuidado semantica, NAO-VALIDADO); unused_index (327, DROP apos 30d/DB-014 - validar idx_scan live, alguns recentes do #43); confirmar SERVICE_ROLE_KEY=JWT (portal-upload-assinatura:53); BLOCKED-Junior: buckets(5)/security_definer_view(41 ERROR)/rotacionar token Telegram. [watch] production_completed 0 lifetime; install_completed 24d; jobs Pendente 18; R$822 CP vencidas.

---
## Ciclo autonomo #45 - 2026-05-29 19:20 BRT - VERDE validar+arrumar (SEC-004): lead_quente filtro 7d CONFIRMADO no runtime (match-count) + search_path fixado em 7 fns SECURITY DEFINER (advisor "65" era inflado - so 7 sao o risco real), fn_claim 200 pos-apply, zero regressao

**Tipo**: validar (runtime, zero-write) + arrumar (1 migration DDL metadata). NOW 19:20 BRT (22:20 UTC); #44 as 18:10 (~56min, sem gatilho passivo). Health VERDE: Vercel 200; edge 60min ZERO 5xx (mcp-bridge-worker v9 ~1/min 200, agent-cron-loop v28 200 ~9-10s); API tick 22:00 UTC limpo ZERO 400/5xx (v28 estavel 8o ciclo); branch=main HEAD da68ce0=#44; guardrail HOST LIMPO (tails STATE 3565/ledger 842/log 1693, 2 untracked herdados, bash NAO consultado).

**TAREFA 1 - lead_quente filtro 7d CONFIRMADO no runtime (fecha watch #44)**: #44 declarou o filtro "limpo" mas NAO checou match-count. 3 queries cruzadas: os 319 hot leads (score>=70 sem proposta) tem updated_at entre 2026-05-05 e 05-12 (newest 17d) -> recalcular_scores NAO bumpa updated_at (minha hipotese de que o recalc derrotaria o filtro REFUTADA pelos dados). filtro updated_at>=now()-7d exclui TODOS os 319 (upd_7d=0). rule_executed lead_quente 24h=100 (TODOS 15:19-15:20 UTC = smoketest #38), 0 desde, last_1h=0. alertas_telegram_dedup envios mais novos=15:19 UTC. Veredito: 0 alertas novos, flood permanece FECHADO; filtro do Junior confirmado eficaz em nivel de match-count.

**TAREFA 2 - SEC-004 search_path hardening (7 SECURITY DEFINER)**: reconciliacao adversarial - advisor function_search_path_mutable=65 (baseline #41) CONFLA SECURITY INVOKER; so 7 sao SECURITY DEFINER em public sem search_path = a real superficie de privilege-escalation. Li os 7 corpos: todas refs non-public QUALIFICADAS (vault., net., pessoal. em format string), unqualified = public-qualif/pg_catalog built-ins. Migration harden_secdef_search_path_cycle45 (DO-block idempotente) ALTER FUNCTION SET search_path=public,pg_temp nos 7. Pos: remaining 7->0, proconfig confirmado, fn_claim_ai_requests 200 em 22:14-22:16 UTC pos-apply (runtime), Vercel 200, zero 5xx.

**Mudancas prod**: 1 migration DDL metadata (7 ALTER FUNCTION SET search_path) - reversivel, sem lock tabela. 0 deploy Edge, 0 prod-data write. Commit planning #45 + migration. Telegram mid=3159.

**NEXT**: auth_rls_initplan (78, janela idle-Junior, NAO-VALIDADO); ~58 SECURITY INVOKER search_path (baixo risco, NAO-VALIDADO); confirmar SERVICE_ROLE_KEY=JWT (portal-upload-assinatura:53); BLOCKED-Junior: buckets(5)/security_definer_view(41)/rotacionar token Telegram. [watch] production_completed 0 lifetime; install_completed 24d; jobs Pendente 18.
...
**Tipo**: explorar + validar (read-only, ZERO prod-write). NOW 15:07 BRT (18:07 UTC); #40 às 14:14 (~53min, sem gatilho passivo). Health 🟢 VERDE: Vercel 200; edge 60min ZERO 5xx (mcp-bridge-worker v9 ~1/min 200, agent-cron-loop v28 200 ~8-9s, dispatch-approved-messages v5 200, ai-detectar-problemas v21 200); API tick 18:00 UTC limpo (system_events 201, execute_sql_readonly 200, GET sentinel recalcular_scores entity_id=00000000 200 — os 3x 400/tick seguem ZERO; v28 estável 4º ciclo); branch=main HEAD 5e48198=#40; guardrail HOST LIMPO (tails STATE 3507 / ledger 768 / log 1613, 3 untracked herdados, bash NÃO consultado p/ corrupção).

**Auto-diálogo (7)**: (1) #38 deploy v28 / #39 achou lead_quente ruído / #40 de-risk SEC-001 + lead_quente dedup holding. (2) Sexta=Instalação exausta #27-34; mcp-bridge-worker v9 saudável. (3) gap útil: P0/P1 NEXT quase todos BLOCKED-Junior (SEC-001 aplicar, token, lead_quente threshold); lead_quente re-validação só ~amanhã 15:20 UTC → pivot pra 2 net-new read-only de alto valor: get_advisors (nunca rodado pelo loop) + auditoria Financeiro (módulo não tocado hoje, domínio Junior). (4) sem conflito IN-PROGRESS/BLOCKED. (5) Obsidian: protocolo Mubisys 05-28 (skip_auto_cr=true) foi DECISIVO pra refutar o agent; secret-leaks 05-27 já rastreados. (6) não-passivo. (7) critério: advisors classificados por nome/level + cross-ref vs SEC-001/conhecido; Financeiro com veredito ✅/⚠️/🔴 por checagem + cross-check das premissas.

**Achados**:
- TAREFA 1 (NET-NEW, get_advisors security+performance — 1ª vez pelo loop): baseline catalogado. SECURITY n=365 (42 ERROR / 322 WARN / 1 INFO): security_definer_view 41 (ERROR), rls_disabled_in_public 1 (ERROR = alertas_telegram_dedup, conhecido #37), rls_policy_always_true 125 (corrobora SEC-001), anon_security_definer_function_executable 62 (VETOR NOVO além do SEC-001 — anon pode EXECUTE 62 fns SECURITY DEFINER), authenticated_security_definer_function_executable 62, function_search_path_mutable 65, public_bucket_allows_listing 5 (exposição STORAGE — NOVO), extension_in_public 2, auth_leaked_password_protection 1, rls_enabled_no_policy 1 (=campo_audit_logs morta). PERFORMANCE n=843 (505 WARN / 338 INFO): multiple_permissive_policies 392, unused_index 327, auth_rls_initplan 78 (antipattern auth.fn() por-linha), duplicate_index 35 (waste puro), unindexed_foreign_keys 11.
- TAREFA 2 (NET-NEW, auditoria adversarial Financeiro via agent isolado + cross-check meu): módulo SAUDÁVEL. ✅ RLS ON nas 6 tabelas (contas_receber/contas_pagar/comissoes/pedidos_compra/fornecedores/lancamentos_caixa); SET ROLE anon = 0 linhas em TODAS (NÃO exposto — contraste positivo c/ SEC-001); 0 FK orphans; 0 double-entry; saldo invariante (valor-pago=saldo) OK. **🔴→✅ FINDING REFUTADO**: agent reportou "receita não faturada R$4.445,71 (4 pedidos aprovado/concluído sem CR)". Cross-check meu (to_jsonb dos 4 pedidos): 1069/1070/PED-2026-0025/PED-2026-0026 são TODOS origem_externa=mubisys + skip_auto_cr=true + skip_auto_op=true + skip_auto_comissao=true → Mubisys mantém cobrança (by-design, protocolo Obsidian 05-28); CR do 1069 soft-deleted 2026-04-14 coerente c/ skip; status_fiscal nao_aplicavel/nao_iniciado. NÃO é receita perdida. Restam 2 ⚠️ menores reais: R$822,00 CP vencidas (2 títulos venc 04-29/05-13 a_pagar — verificar baixa externa); lancamentos_caixa/comissoes/parcelas/lancamentos_contabeis = 0 linhas (módulos dormentes, provável by-design).

**Mudanças prod**: NENHUMA (read-only). 0 deploy / 0 migration / 0 prod-write. Entregas: 3 cérebros + Obsidian + advisors baseline → ledger NEXT (SEC-002 novo + perf wins).

**Anti-pattern evitado**: CROSS-CHECK do output do AGENT (não confiei cego) → refutei R$4.445,71 com query (skip_auto_cr=true / origem_externa=mubisys). NÃO apliquei advisors cego (revogar EXECUTE/views ou perf-migrations pode quebrar app → exige janela+análise). NÃO Cowork Edit nos cérebros (>250 LOC; via HOST .NET UTF8). NÃO declarei Financeiro "ok" sem rodar SET ROLE anon + counts cruzados.
...
### 🔴 SEC-001 - EXPOSICAO ANON (runtime-provada, nao inspecao estatica)
SET ROLE anon + count: anon (chave publica do frontend, SEM login) LE leads=3460, ai_alertas=357, clientes=336, produtos=107, telegram_messages=42, regras_precificacao=11. Controles get_user_role()-gated (pedidos/contas_receber/jobs) = 0 -> PROVA que o vetor sao as policies TO public USING(true), nao bypass. anon tem GRANT ALL nas 181 tabelas (default Supabase) entao RLS e o unico gate. "37 tabelas RLS off" de abril MAL ROTULADO: real = 1 RLS off (alertas_telegram_dedup interna) + 37 policies role public (das quais ~6-10 OPEN(true) em tabela sensivel; resto auth-gated OK, provado pelos controles=0). PII de 3460 leads + 336 clientes + conteudo telegram legivel com anon key embutida no JS -> risco LGPD. NAO corrigido em prod (business hours; mudar policy de core/catalogo pode quebrar ERP/portal; exposicao de meses). Migration VALIDADA parqueada + BLOCKED Junior.

### ✅ INT-001 REFUTADO + 🔴 INT-005 PARCIAL
INT-001 (cron JWT hardcoded de abril): 14 crons, 0 com JWT literal; Edge-callers usam 'Bearer '||private.get_service_role_key() vault-backed (sec_def, uses_vault=true). Obsoleto. INT-005: notificar-aprovacao-telegram CORRIGIDO (v2-vault-token). MAS telegram-webhook/index.ts:11 ainda com token Telegram HARDCODED (mesmo token 05-27; 1 de >=2 arquivos coberto pelo fix do Junior). mcp-server anon key hardcoded (LOW, chave publica). RPC vault get_telegram_bot_token ja existe. Remediacao = ROTACIONAR via @BotFather (Junior) + vault. BLOCKED Junior.

### Entregas + NEXT
Zero prod write/deploy/migration-aplicada. Docs: planning/SEC-AUDIT-2026-05-29-anon-exposure.md + planning/SEC-001-remediacao-anon-rls-VALIDADA.sql (idempotente, NAO-aplicada). NEXT (ledger #37): P0 aplicar remediacao RLS em janela monitorada (Bloco1 leads/clientes TO authenticated + Bloco2 DROP service_role_manage_alertas; confirmar portal/anon nao le antes) + P0 rotacionar token telegram; P1 herdado deploy v28 (2 fixes validados); P2 revogar GRANT anon redundante. Watch production_completed 0 lifetime / install_completed 24d / jobs Pendente 18 / followup_engine_ativo=false.

---

## Ciclo autonomo #36 - 2026-05-29 10:07 BRT - 3x 400/tick do agent-cron-loop ROOT-CAUSADOS (2 fixes VALIDADOS) + reschedule v27 drenando confirmado VERDE

**Mantra**: explorar + validar. Sexta=Instalacao mas JA auditada 8x hoje (#27-34) -> pivot pro P2 do #35 NEXT nunca investigado (3x 400/tick do cron, runtime-live no log API). Hora 10:07 BRT (#35 as 08:30, ~1h37, sem gatilho passivo). Health pre VERDE: Vercel 200, edge 60min ZERO 5xx (mcp-bridge-worker v9 ~1/min, agent-cron-loop v27 200 ~8-10s, dispatch v5 200), API 0 5xx (os 400 sao 4xx client = o achado), branch=main HEAD ff5409e=#35, guardrail HOST LIMPO (3 untracked herdados, 0 modified, tails 3428/719/1496 L). 1 agent isolado read-only (70k) + 2 SQL inline.

**Achados (runtime-provados, nao inspecao estatica)**:
- system_events.entity_id = uuid NOT NULL (info_schema).
- recalcular_scores (index.ts L526) retorna id='batch' -> entity_id na idempotency-GET L582 + INSERT rule_executed L637-643 -> 'batch'::uuid da 22P02 -> 2 system_events 400/tick. Efeito: rule_executed dessa regra NUNCA grava (dedupe quebrado) + fn_recalcular_todos_scores RE-EXECUTA a cada 30min.
- lead_quente_sem_orcamento (index.ts L478) usa cl.lead_origem_id (inexistente; real clientes.lead_id) -> execute_sql_readonly 42703 400/tick -> regra 100% MORTA (0 match, 0 automacao). Cross-ref: mesma coluna do #9 "corrigida" no #10 SO no data-layer; copia hardcoded no source escapou. "fix aplicado != funcionou".

**Fixes**: 2 linhas, [VALIDADOS] contra schema, no NEXT do ledger. NAO deployei: Junior shippou v27 hoje cedo e esta ativo no arquivo; bugs de semanas (sem regressao em esperar); 1369 LOC = agent isolado/Claude Code. Zero deploy/migration/prod-write.

**Validacao bonus (P1 #35)**: reschedule v27 DRENANDO stuck-pool - eligible_followups 195(#32)->180(#35)->135; tentativas_zero 152->108; oldest_due 05-11->05-14. SEND gated OFF (followup_engine_ativo=false). Watch: jobs Pendente 18, install_completed 24d (INSTAL-01), production_completed 0 lifetime (chain 4195dc7 dormente).
