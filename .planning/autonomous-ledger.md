...
- [ ] **🔥 NOVO P1 SAFE (reformulado pós-incidente #14) — DEFAULT AUTÔNOMO**: deploy v25 de agent-cron-loop com fix `.insert(...).catch is not a function` + `getLegacyJwt()` cacheado. **Abordagem segura sem Edit em arquivo > 500 LOC**: (1) criar arquivo NOVO `supabase/functions/ai-shared/safe-insert.ts` (≤80 LOC) — helper `safeInsert(supabase, table, payload)` com try/catch + console.warn; (2) criar arquivo NOVO `supabase/functions/ai-shared/legacy-jwt.ts` (≤80 LOC) — `getLegacyJwt()` com cache no isolate + refresh sob 401 (extrair de `mcp-bridge-worker/index.ts` linhas 14-22/144-177); (3) **Edit MÍNIMO no agent-cron-loop**: trocar APENAS as 4 linhas de `import` no topo do arquivo (adicionar 2 imports novos) e substituir SOMENTE strings curtas e únicas como `.catch(() => {})` por `// caught by safeInsert` — usar `replace_all=true` se string for única, ou Edit cirúrgico com contexto mínimo. (4) Se Edit ainda parecer arriscado: DELEGAR a agent isolado com instrução "use Write tool em ai-shared/* mas para agent-cron-loop/index.ts use APENAS Edit cirúrgico com no máx 30 linhas de contexto, NUNCA reescrever o arquivo inteiro". (5) **Tail-check obrigatório** pós-Edit: `wc -l` deve continuar 1230 ± 5, `tail -3` deve terminar em `return true;\n}`. Janela flexível (Edge interna). **NÃO TENTAR antes do hardening do guardrail Etapa 4 abaixo**.
...
- [ ] **NOVO ciclo #13 — P2 DEFAULT AUTÔNOMO**: investigar 17 chamadas `POST | 401 | ai-compor-mensagem` durante smoketest. agent-cron-loop chama via `supabase.functions.invoke('ai-compor-mensagem', { body: ..., headers: { 'X-Internal-Call': 'true' } })`. Suspeita: `verify_jwt:true` na ai-compor-mensagem com client supabase-js usando anon key, header X-Internal-Call não basta. Solução possível: passar Authorization explícito ou setar `verify_jwt:false` em ai-compor-mensagem (a Edge faria seu próprio check de X-Internal-Call). Read-only auditoria primeiro.
