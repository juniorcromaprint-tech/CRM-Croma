...
- [ ] **🔥 NOVO ciclo #13 — P1 DEFAULT AUTÔNOMO**: deploy v25 de agent-cron-loop com fix `.insert(...).catch is not a function`. Estratégia: helper local `safeInsert(supabase, table, payload, opts?)` wrap try/catch + console.warn em vez de `.catch(() => {})`. Substituir todos os sites (linhas 174-183 ai_logs success, 232-239 ai_logs error, outros). Mesma classe de bug que ciclo #6 corrigiu em ai-chat-portal v15. **CUIDADO**: arquivo 1230 linhas — delegar a agent isolado (Edit Cowork trunca > 500 LOC). Janela flexível (Edge interna).
