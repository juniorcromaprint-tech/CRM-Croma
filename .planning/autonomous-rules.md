...
2. **BACKLOG DE MÓDULOS** (`BACKLOG-MODULOS-2026-05-29.md`) ← **DRIVER PRIMÁRIO**. Pegar o próximo item `default-exec` na ordem de lane (P0 Correção de dinheiro → P1 Fiscal/segurança código → P2 Infra). Respeitar `[NAO-VALIDADO]` (validar contra schema antes de fechar) e janelas de Edge cliente. ⛔ **OBRIGATÓRIO (lição #49)**: se há item P0 `default-exec` aplicável no backlog, o ciclo DEVE pegá-lo OU citar o backlog explicitamente no auto-diálogo (Etapa 2) + justificar por que outro achado o substituiu (só vale: 5xx ativo, EMERGÊNCIA, ou bug que quebra o próprio ciclo). **Pular o backlog em SILÊNCIO — sem citá-lo — é PROIBIDO** (#49 corrigiu Memory Layer mas ignorou DB-012 sem justificar). Free-style só depois do P0 consumido ou justificado.
