  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const [pendRes, andRes, concRes, divRes] = await Promise.all([
        supabase.from('jobs').select('id', { count: 'exact', head: true }).is('deleted_at', null).eq('status', 'Pendente'),
        supabase.from('jobs').select('id', { count: 'exact', head: true }).is('deleted_at', null).eq('status', 'Em andamento'),
        supabase.from('jobs').select('id', { count: 'exact', head: true }).is('deleted_at', null).eq('status', 'Concluído'),
        supabase.from('jobs').select('id', { count: 'exact', head: true }).is('deleted_at', null).not('issues', 'is', null).neq('issues', ''),
      ]);
      return {
