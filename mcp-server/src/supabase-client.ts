...
let _juniorUserId: string | null = null;
 ...
 */
export function getJuniorUserId(): string | null {
...
export async function initUserAuth(): Promise<void> {
  ...
  if (!password) {
    process.stderr.write(
      "[croma-mcp] ⚠️  SUPABASE_USER_PASSWORD não configurado.\n" +
  ...
  if (error || !data.user) {
    ...
    process.stderr.write(
      `[croma-mcp] ❌ Falha ao autenticar como ${email}: ${msg}\n` +
  ...
  process.stderr.write(
    `[croma-mcp] ✅ Autenticado como ${email}\n` +
