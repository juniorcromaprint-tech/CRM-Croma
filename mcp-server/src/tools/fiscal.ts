...
export function registerFiscalTools(server: McpServer): void {
  // ─── croma_listar_nfe ──────────────────────────────────────────────────────
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
    ...
    async (params) => {
      try {
        ...
        if (params.tipo_documento) query = query.eq("tipo_documento", params.tipo_documento);
        ...
        if (params.response_format === ResponseFormat.MARKDOWN) {
          ...
          if (items.length === 0) {
            ...
            for (const doc of items) {
              const pedido = (doc.pedidos as { numero?: string }) ?? {};
              const cliente = (doc.clientes as { razao_social?: string; nome_fantasia?: string }) ?? {};
              ...
              if (doc.data_emissao) lines.push(`- **Emissão**: ${formatDate(doc.data_emissao)}`);
              if (doc.data_autorizacao) lines.push(`- **Autorização**: ${formatDate(doc.data_autorizacao)}`);
              if (doc.chave_acesso) lines.push(`- **Chave**: \`${doc.chave_acesso.slice(0, 20)}...\``);
              if (doc.mensagem_erro) lines.push(`- **Erro**: ${doc.mensagem_erro}`);
              lines.push("");
            }
  ...
  - pedido_id (string, obrigatório): UUID do pedido para emitir NF-e`,
    ...
    async (params) => {
      try {
        ...
        if (!serviceRoleKey) return { content: [{ type: "text" as const, text: "❌ SUPABASE_SERVICE_ROLE_KEY não configurada." }] };

        const response = await fetch(
          `${SUPABASE_URL}/functions/v1/fiscal-emitir-nfe`,
          {
            method: "POST",
            headers: {
  ...
  - documento_id (string, obrigatório): UUID do documento fiscal (fiscal_documentos.id)`,
    ...
    async (params) => {
      try {
        ...
        const response = await fetch(
          `${SUPABASE_URL}/functions/v1/fiscal-consultar-nfe`,
          {
            method: "POST",
            headers: {
              "Authorization": `Bearer ${serviceRoleKey}`,
