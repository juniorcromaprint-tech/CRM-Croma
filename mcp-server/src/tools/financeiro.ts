  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
    ...
    async (params) => {
      try {
        ...
        if (params.filtro_categoria) query = query.ilike("categoria", `%${params.filtro_categoria}%`);
  ...
  - observacoes (string, opcional): Instruções adicionais no boleto`,
    ...
    async (params) => {
      try {
        ...
        if (crErr) return errorResult(crErr);
        if (!cr) return { content: [{ type: "text" as const, text: `Conta a receber não encontrada: ${params.conta_receber_id}` }] };
        ...
        if (!bancoId) {
          ...
          if (!bancos || bancos.length === 0) {
            return { content: [{ type: "text" as const, text: "Nenhuma conta bancária ativa cadastrada. Cadastre uma conta bancária primeiro." }] };
          }
          bancoId = bancos[0].id;
        }

        const cliente = cr.clientes as { razao_social?: string; cnpj?: string; endereco?: string; cidade?: string; estado?: string; cep?: string } | null;
        ...
        if (boletoErr) return errorResult(boletoErr);
        ...
        if (params.response_format === ResponseFormat.MARKDOWN) {
          ...
          if (items.length === 0) {
            ...
            for (const d of items) {
              ...
              if (d.data_pagamento) lines.push(`- **Pago em**: ${formatDate(d.data_pagamento)}`);
              lines.push("");
            }
          }
          text = lines.join("\n");
