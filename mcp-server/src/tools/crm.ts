    ...
    async (params) => {
      try {
        ...
        if (cnpjDigits.length !== 14) {
          return { content: [{ type: "text" as const, text: `❌ CNPJ inválido — deve ter 14 dígitos. Recebido: ${params.cnpj}` }] };
        }
        ...
        if (!dados.razao_social) {
          return { content: [{ type: "text" as const, text: `❌ CNPJ ${formatted} não encontrado na Receita Federal. Cadastre manualmente via croma_cadastrar_cliente.` }] };
        }
