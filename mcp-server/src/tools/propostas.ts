  ...
  - itens_remover (array de UUIDs, opcional): IDs dos itens a remover`,
    ...
    async (params) => {
      try {
        ...
        if (fetchError) return errorResult(fetchError);
        if (!proposta) return { content: [{ type: "text" as const, text: `❌ Proposta não encontrada: ${params.id}` }] };
        ...
        if (params.titulo !== undefined) { propostaUpdate.titulo = params.titulo; alteracoes.push(`Título → "${params.titulo}"`); }
        ...
        if (params.condicoes_pagamento !== undefined) { propostaUpdate.condicoes_pagamento = params.condicoes_pagamento; alteracoes.push("Condições de pagamento atualizadas"); }
        if (params.observacoes !== undefined) { propostaUpdate.observacoes = params.observacoes; alteracoes.push("Observações atualizadas"); }
        if (params.desconto_percentual !== undefined) { propostaUpdate.desconto_percentual = params.desconto_percentual; alteracoes.push(`Desconto → ${params.desconto_percentual}%`); }
        ...
        if (params.itens_atualizar && params.itens_atualizar.length > 0) {
          for (const item of params.itens_atualizar) {
            ...
            if (item.descricao !== undefined) itemUpdate.descricao = item.descricao;
            if (item.quantidade !== undefined) itemUpdate.quantidade = item.quantidade;
            if (item.valor_unitario !== undefined) itemUpdate.valor_unitario = item.valor_unitario;
            if (item.largura_cm !== undefined) itemUpdate.largura_cm = item.largura_cm;
            if (item.altura_cm !== undefined) itemUpdate.altura_cm = item.altura_cm;
            if (item.material_descricao !== undefined) itemUpdate.material_descricao = item.material_descricao;
        ...
        const cliente = proposta.clientes as unknown as { razao_social: string; nome_fantasia?: string } | null;
