...
export function registerBiTools(server: McpServer): void {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
    ...
    async (params) => {
      try {
        ...
        const alertas: Array<{ tipo: string; nivel: string; mensagem: string; detalhe?: unknown }> = [];

        // Pedidos atrasados
        for (const p of pedidosAtrasados.data ?? []) {
          const cliente = (p.clientes as { razao_social?: string }) ?? {};
          alertas.push({
        ...
        if (contasVencidas.data && contasVencidas.data.length > 0) {
          alertas.push({
            ...
            detalhe: contasVencidas.data.slice(0, 3).map(c => ({
              ...
              cliente: (c.clientes as { razao_social?: string })?.razao_social,
        ...
        if (params.response_format === ResponseFormat.MARKDOWN) {
          if (alertas.length === 0) {
            ...
            if (criticos.length > 0) {
              ...
              for (const a of criticos) lines.push(`- ${a.mensagem}`);
              lines.push("");
            }
            if (altos.length > 0) {
              lines.push(`### 🟠 Altos (${altos.length})`);
              for (const a of altos) lines.push(`- ${a.mensagem}`);
              lines.push("");
            }
            if (medios.length > 0) {
              lines.push(`### 🟡 Médios (${medios.length})`);
              for (const a of medios) lines.push(`- ${a.mensagem}`);
            }
            text = lines.join("\n");
          }
        } else {
        ...
        return {
          content: [{ type: "text" as const, text: truncateIfNeeded(text, alertas.length) }],
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
    ...
    async (params) => {
      try {
        ...
        const propostasPorStatus = propostas.reduce<Record<string, { count: number; total: number }>>((acc, p) => {
        ...
        const pedidosPorStatus = pedidos.reduce<Record<string, { count: number; total: number }>>((acc, p) => {
          if (!acc[p.status]) acc[p.status] = { count: 0, total: 0 };
          acc[p.status].count++;
