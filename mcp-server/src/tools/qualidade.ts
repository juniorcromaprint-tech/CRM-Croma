...
export function registerQualidadeTools(server: McpServer): void {
    ...
    async (params) => {
      try {
        ...
        let query = sb
          .from("ocorrencias")
          .select(
            `id, tipo, status, descricao, causa, custo_total, impacto_prazo_dias, created_at,
             pedidos(numero)`,
            { count: "exact" }
          );
        ...
        if (params.response_format === ResponseFormat.MARKDOWN) {
          ...
          if (items.length === 0) {
            ...
            for (const oc of items) {
              const pedido = (oc.pedidos as { numero?: string }) ?? {};
              ...
              if (oc.causa) lines.push(`- **Causa**: ${oc.causa?.replace(/_/g, " ")}`);
              if (oc.custo_total && oc.custo_total > 0) lines.push(`- **Custo**: ${formatBRL(oc.custo_total)}`);
              if (oc.impacto_prazo_dias) lines.push(`- **Impacto prazo**: ${oc.impacto_prazo_dias} dias`);
...
Args:
  ...
  - impacto_prazo_dias (number, opcional): Dias de atraso gerados`,
      inputSchema: z.object({
        ...
        custo_mp: z.number().min(0).optional(),
        custo_mo: z.number().min(0).optional(),
        impacto_prazo_dias: z.number().int().min(0).optional(),
      }).strict(),
  ...
  - observacoes (string, opcional): Observações sobre a resolução`,
    ...
    async (params) => {
      try {
        ...
        if (!atual) return { content: [{ type: "text" as const, text: `Ocorrência não encontrada: ${params.id}` }] };
