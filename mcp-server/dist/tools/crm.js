...
import { formatBRL, formatDate, formatCNPJ, formatPhone, formatStatus, } from "../utils/formatting.js";
// ─── croma_listar_clientes ───────────────────────────────────────────────────
export function registerCrmTools(server) {
    server.registerTool("croma_listar_clientes", {
...
cidade, estado, classificacao, segmento, vendedor_id, limite_credito, ativo`,
        ...
        try {
            ...
            let query = sb
                .from("clientes")
                .select("id, razao_social, nome_fantasia, cnpj, telefone, email, cidade, estado, classificacao, segmento, ativo, limite_credito, created_at", { count: "exact" });
            ...
            if (params.estado)
                query = query.eq("estado", params.estado.toUpperCase());
            if (params.busca) {
                query = query.or(`razao_social.ilike.%${params.busca}%,nome_fantasia.ilike.%${params.busca}%,cnpj.ilike.%${params.busca}%`);
            }
            query = query
                .order("razao_social")
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const c of items) {
                        ...
                        if (c.telefone)
                            lines.push(`- **Telefone**: ${formatPhone(c.telefone)}`);
                        if (c.email)
                            lines.push(`- **Email**: ${c.email}`);
                        if (c.cidade)
                            lines.push(`- **Cidade**: ${c.cidade}${c.estado ? `/${c.estado}` : ""}`);
                        if (c.classificacao)
                            lines.push(`- **Classificação**: ${c.classificacao}`);
                        if (c.segmento)
                            lines.push(`- **Segmento**: ${c.segmento}`);
                        if (c.limite_credito)
                            lines.push(`- **Limite de Crédito**: ${formatBRL(c.limite_credito)}`);
    ...
    server.registerTool("croma_detalhe_cliente", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Formato da resposta (padrão: markdown)`,
        ...
        try {
            const sb = getAdminClient();
            const [clienteResult, contatosResult, unidadesResult, propostasResult, pedidosResult] = await Promise.all([
            ...
            if (clienteResult.error)
                return errorResult(clienteResult.error);
            if (!clienteResult.data) {
                return { content: [{ type: "text", text: `Cliente não encontrado: ${params.id}` }] };
            }
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (contatos.length > 0) {
                    ...
                    for (const ct of contatos) {
                        ...
                        if (ct.cargo)
                            lines.push(`- **Cargo**: ${ct.cargo}`);
                        if (ct.telefone)
                            lines.push(`- **Telefone**: ${formatPhone(ct.telefone)}`);
                        if (ct.whatsapp)
                            lines.push(`- **WhatsApp**: ${formatPhone(ct.whatsapp)}`);
                        if (ct.email)
                            lines.push(`- **Email**: ${ct.email}`);
                        if (ct.e_decisor)
                            lines.push("- **É decisor**: ✅");
                        lines.push("");
                    }
                }
                if (unidades.length > 0) {
                    ...
                    for (const u of unidades) {
                        lines.push(`### ${u.nome}`);
                        if (u.cidade)
                            lines.push(`- **Cidade**: ${u.cidade}/${u.estado ?? ""}`);
                        if (u.endereco)
                            lines.push(`- **Endereço**: ${u.endereco}`);
                        if (u.contato_local)
                            lines.push(`- **Contato local**: ${u.contato_local}`);
                        lines.push("");
                    }
                }
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, 1) }],
    ...
    server.registerTool("croma_cadastrar_cliente", {
...
Retorna: dados do cliente criado com ID gerado`,
    ...
    server.registerTool("croma_atualizar_cliente", {
  ...
  - limite_credito (number, opcional): Novo limite de crédito em R$`,
        ...
        try {
            ...
            if (Object.keys(updates).length === 0) {
                return { content: [{ type: "text", text: "Nenhum campo para atualizar foi informado." }] };
            }
            const sb = getUserClient();
            const { data, error } = await sb
                .from("clientes")
            ...
            if (error)
                return errorResult(error);
            if (!data)
                return { content: [{ type: "text", text: `Cliente não encontrado: ${id}` }] };
            return {
                content: [{
                        type: "text",
    ...
    // ─── croma_listar_leads ────────────────────────────────────────────────────
    server.registerTool("croma_listar_leads", {
        title: "Listar Leads",
...
Args:
  ...
  - response_format ('markdown'|'json'): Formato (padrão: markdown)`,
        ...
        try {
            ...
            if (params.segmento)
                query = query.ilike("segmento", `%${params.segmento}%`);
            if (params.score_min !== undefined)
                query = query.gte("score", params.score_min);
            if (params.busca) {
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const l of items) {
                        ...
                        lines.push(`- **ID**: \`${l.id}\``);
                        if (l.contato_nome)
                            lines.push(`- **Contato**: ${l.contato_nome}`);
                        if (l.telefone)
                            lines.push(`- **Telefone**: ${formatPhone(l.telefone)}`);
                        if (l.email)
                            lines.push(`- **Email**: ${l.email}`);
                        lines.push(`- **Status**: ${formatStatus(l.status)}`);
                        if (l.score)
                            lines.push(`- **Score**: ${l.score}/100`);
                        if (l.segmento)
                            lines.push(`- **Segmento**: ${l.segmento}`);
    ...
    server.registerTool("croma_cadastrar_lead", {
...
Args:
  ...
  - score (number, opcional): Score inicial 0-100 (padrão: 50)`,
    ...
    server.registerTool("croma_atualizar_status_lead", {
...
Args:
  ...
  - observacao (string, opcional): Motivo da mudança`,
        ...
        try {
            ...
            if (!atual)
                return { content: [{ type: "text", text: `Lead não encontrado: ${params.id}` }] };
            const transicoes = {
                novo: ["em_contato", "contatado", "descartado"],
                em_contato: ["contatado", "qualificando", "descartado"],
                contatado: ["qualificando", "descartado"],
            ...
            if (params.observacao)
                updateData.observacoes = params.observacao;
    ...
    server.registerTool("croma_listar_atividades_comerciais", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            if (params.filtro_entidade_id)
                query = query.eq("entidade_id", params.filtro_entidade_id);
            if (params.filtro_entidade_tipo)
                query = query.eq("entidade_tipo", params.filtro_entidade_tipo);
            if (params.filtro_tipo)
                query = query.eq("tipo", params.filtro_tipo);
            if (params.data_inicio)
                query = query.gte("data_atividade", params.data_inicio);
            if (params.data_fim)
                query = query.lte("data_atividade", params.data_fim + "T23:59:59");
    ...
    server.registerTool("croma_registrar_atividade_comercial", {
...
Args:
  ...
  - proximo_passo (string, opcional): Próxima ação prevista`,
    ...
    server.registerTool("croma_listar_comissoes", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                ...
                .is("excluido_em", null);
            if (params.filtro_vendedor)
                query = query.eq("vendedor_id", params.filtro_vendedor);
            if (params.filtro_status)
                query = query.eq("status", params.filtro_status);
            if (params.data_inicio)
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const c of items) {
                        const vendedor = c.profiles?.full_name ?? c.vendedor_id;
    ...
    server.registerTool("croma_registrar_comissao", {
...
Args:
  ...
  - observacoes (string, opcional): Observações`,
    ...
    server.registerTool("croma_listar_contratos", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                ...
                .is("excluido_em", null);
            if (params.filtro_cliente)
                query = query.eq("cliente_id", params.filtro_cliente);
            if (params.filtro_status)
                query = query.eq("status", params.filtro_status);
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const c of items) {
                        ...
                        if (c.periodicidade)
                            lines.push(`- **Periodicidade**: ${c.periodicidade}`);
                        lines.push(`- **Início**: ${formatDate(c.data_inicio)}`);
                        if (c.data_fim)
                            lines.push(`- **Fim**: ${formatDate(c.data_fim)}`);
                        if (c.proximo_faturamento)
                            lines.push(`- **Próximo faturamento**: ${formatDate(c.proximo_faturamento)}`);
                        lines.push("");
                    }
                }
                text = lines.join("\n");
    ...
    server.registerTool("croma_criar_contrato", {
...
Args:
  ...
  - observacoes (string, opcional): Observações`,
    ...
    server.registerTool("croma_listar_campanhas", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const c of items) {
                        ...
                        if (c.data_inicio)
                            lines.push(`- **Período**: ${formatDate(c.data_inicio)}${c.data_fim ? ` a ${formatDate(c.data_fim)}` : ""}`);
                        if (c.orcamento)
                            lines.push(`- **Orçamento**: ${formatBRL(c.orcamento)}`);
    ...
    // ─── croma_listar_nps ─────────────────────────────────────────────────────
    server.registerTool("croma_listar_nps", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                ...
                .not("nota", "is", null);
            if (params.filtro_nota !== undefined)
                query = query.eq("nota", params.filtro_nota);
            if (params.data_inicio)
                query = query.gte("respondido_em", params.data_inicio);
            if (params.data_fim)
                query = query.lte("respondido_em", params.data_fim + "T23:59:59");
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const r of items.slice(0, 20)) {
                        const cliente = r.clientes?.razao_social ?? r.cliente_id ?? "—";
