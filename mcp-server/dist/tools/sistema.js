...
function validateReadOnlySQL(sql) {
    ...
    for (const keyword of SQL_FORBIDDEN_KEYWORDS) {
        ...
        if (regex.test(upperSQL)) {
            throw new Error(`Query rejeitada: contém operação proibida "${keyword}". ` +
    ...
    if (!upperSQL.trim().startsWith("SELECT") && !upperSQL.trim().startsWith("WITH")) {
        throw new Error("Query rejeitada: deve iniciar com SELECT ou WITH. " +
            "Apenas queries de leitura são permitidas.");
    }
}
export function registerSistemaTools(server) {
    // ─── croma_executar_sql ────────────────────────────────────────────────────
    server.registerTool("croma_executar_sql", {
...
estoque_saldos, estoque_movimentos, profiles, fornecedores`,
        ...
        try {
            ...
            const { data, error } = await sb.rpc("execute_sql_readonly", { query_text: sql });
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, rows.length) }],
    ...
    // ─── croma_health_check ────────────────────────────────────────────────────
    server.registerTool("croma_health_check", {
        ...
        try {
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                text = [
                    ...
                    "",
                    "## Tabelas Principais",
                    ...tabelas.map(t => `- ${t.ok ? "✅" : "❌"} **${t.nome}**: ${t.count !== null ? t.count?.toLocaleString("pt-BR") : "erro"}`),
                ].join("\n");
            }
            else {
