...
export function registerEstoqueTools(server) {
    // ─── croma_consultar_estoque ───────────────────────────────────────────────
    server.registerTool("croma_consultar_estoque", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                .from("materiais")
                .select(`id, codigo, nome, categoria, unidade, preco_medio, estoque_minimo, ativo,
             estoque_saldos(quantidade_disponivel, quantidade_reservada)`, { count: "exact" })
                .eq("ativo", true);
            if (params.busca) {
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const m of items) {
                        const saldoObj = m.estoque_saldos?.[0] ?? {};
                        ...
                        if (m.codigo)
                            lines.push(`- **Código**: ${m.codigo}`);
                        if (m.categoria)
                            lines.push(`- **Categoria**: ${m.categoria}`);
                        lines.push(`- **Disponível**: ${disponivel} ${m.unidade ?? ""}`);
                        if (reservado > 0)
                            lines.push(`- **Reservado**: ${reservado} ${m.unidade ?? ""}`);
                        if (m.estoque_minimo)
                            lines.push(`- **Mínimo**: ${m.estoque_minimo} ${m.unidade ?? ""}${alertaMinimo ? " ⚠️" : ""}`);
                        if (m.preco_medio)
                            lines.push(`- **Preço médio**: ${formatBRL(m.preco_medio)}/${m.unidade ?? "un"}`);
                        lines.push("");
                    }
    ...
    server.registerTool("croma_listar_materiais", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                for (const m of items) {
                    ...
                    if (m.preco_medio)
                        lines.push(`  - Preço: ${formatBRL(m.preco_medio)}/${m.unidade ?? "un"}`);
                }
                if (response.has_more)
    ...
    server.registerTool("croma_cadastrar_material", {
...
Retorna: dados do material criado com ID gerado`,
    ...
    server.registerTool("croma_atualizar_material", {
  ...
  - aproveitamento (number, opcional): % de aproveitamento`,
        ...
        try {
            ...
            if (error)
                return errorResult(error);
            if (!data)
                return { content: [{ type: "text", text: `Material não encontrado: ${material_id}` }] };
            return {
                content: [{
                        type: "text",
    ...
    server.registerTool("croma_sugerir_compra", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            const { data, error } = await sb
                .from("materiais")
                .select(`id, codigo, nome, categoria, unidade, preco_medio, estoque_minimo, estoque_ideal, estoque_controlado,
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (lista.length === 0) {
                    ...
                    for (const m of lista) {
                        ...
                        if (m.preco_medio)
                            lines.push(`- **Valor estimado**: ${formatBRL(m.valorEstimado)}`);
                        lines.push("");
                    }
                }
                text = lines.join("\n");
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, lista.length) }],
    ...
    server.registerTool("croma_historico_precos_material", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            if (matResult.error)
                return errorResult(matResult.error);
            if (!matResult.data) {
                return { content: [{ type: "text", text: `Material não encontrado: ${params.material_id}` }] };
            }
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, historico.length) }],
    ...
    server.registerTool("croma_registrar_movimento", {
