...
export function registerPedidosTools(server) {
    // ─── croma_listar_pedidos ──────────────────────────────────────────────────
    server.registerTool("croma_listar_pedidos", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                .from("pedidos")
                .select(`id, numero, status, prioridade, valor_total, data_prometida, created_at,
             clientes!inner(id, razao_social, nome_fantasia)`, { count: "exact" });
            if (params.status)
            ...
            if (params.atrasados)
                query = query.lt("data_prometida", new Date().toISOString()).not("status", "in", '("entregue","faturado","cancelado")');
                ...
                if (items.length === 0) {
                    ...
                    for (const p of items) {
                        ...
                        if (p.prioridade && p.prioridade !== "normal")
                            lines.push(`- **Prioridade**: ${p.prioridade === "urgente" ? "🔴 Urgente" : "🟡 Alta"}`);
                        lines.push(`- **Valor**: ${formatBRL(p.valor_total)}`);
                        if (p.data_prometida)
                            lines.push(`- **Prazo**: ${formatDate(p.data_prometida)}${atrasado ? " ⚠️" : ""}`);
    ...
    server.registerTool("croma_detalhe_pedido", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            if (pedidoResult.error)
                return errorResult(pedidoResult.error);
            if (!pedidoResult.data)
                return { content: [{ type: "text", text: `Pedido não encontrado: ${params.id}` }] };
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (p.observacoes)
                    lines.push(`**Observações**: ${p.observacoes}\n`);
                if (itens.length > 0) {
                ...
                if (ops.length > 0) {
                    ...
                    for (const op of ops) {
                        ...
                        if (op.prazo_interno)
                            lines.push(`- **Prazo interno**: ${formatDate(op.prazo_interno)}`);
                        if (op.tempo_estimado_min)
                            lines.push(`- **Tempo estimado**: ${op.tempo_estimado_min} min`);
                        lines.push("");
                    }
                }
                text = lines.join("\n");
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, itens.length + ops.length + 1) }],
    ...
    server.registerTool("croma_listar_ordens_producao", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                ...
                .select(`id, numero, status, prioridade, prazo_interno, tempo_estimado_min,
             setor_atual_id, responsavel_id, created_at,
             pedidos!inner(numero, clientes(razao_social, nome_fantasia))`, { count: "exact" });
            if (params.status)
                query = query.eq("status", params.status);
            if (params.pedido_id)
                query = query.eq("pedido_id", params.pedido_id);
            if (params.setor)
                query = query.eq("setor_atual_id", params.setor);
            if (params.responsavel_id)
                query = query.eq("responsavel_id", params.responsavel_id);
            if (params.atrasadas)
                query = query.lt("prazo_interno", new Date().toISOString()).not("status", "in", '("finalizado","liberado")');
            query = query
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const op of items) {
                        ...
                        lines.push(`- **Status**: ${formatStatus(op.status)}`);
                        if (op.setor_atual_id)
                            lines.push(`- **Setor ID**: ${op.setor_atual_id}`);
                        if (op.prazo_interno)
                            lines.push(`- **Prazo**: ${formatDate(op.prazo_interno)}${atrasada ? " ⚠️ VENCIDO" : ""}`);
                        if (op.tempo_estimado_min)
                            lines.push(`- **Tempo est.**: ${op.tempo_estimado_min} min`);
                        lines.push("");
                    }
                    if (response.has_more)
                        lines.push(`_Mais resultados. Use offset: ${response.next_offset}._`);
    ...
    server.registerTool("croma_atualizar_status_producao", {
...
Args:
  ...
  - responsavel_id (string, opcional): UUID do responsável`,
        ...
        try {
            ...
            if (!atual)
                return { content: [{ type: "text", text: `OP não encontrada: ${params.id}` }] };
            const transicoes = {
                aguardando_programacao: ["em_fila"],
                em_fila: ["em_producao"],
                em_producao: ["em_acabamento", "retrabalho"],
    ...
    server.registerTool("croma_atualizar_status_pedido", {
  ...
  - observacao (string, opcional): Observação sobre a mudança`,
        ...
        try {
            ...
            if (params.status === "cancelado") {
                ...
                if (params.observacao)
                    updateData.motivo_cancelamento = params.observacao;
            }
    ...
    server.registerTool("croma_criar_ordem_producao", {
  ...
  - observacoes (string, opcional): Observações`,
        ...
        try {
            ...
            const { data: pedido, error: pedidoErr } = await sb
                ...
                .single();
            if (pedidoErr)
                return errorResult(pedidoErr);
            if (!pedido)
                return { content: [{ type: "text", text: `Pedido não encontrado: ${params.pedido_id}` }] };
    ...
    server.registerTool("croma_registrar_apontamento_producao", {
...
Args:
  ...
  - observacoes (string, opcional): Observações`,
    ...
    server.registerTool("croma_listar_apontamentos_producao", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            if (params.ordem_producao_id)
                query = query.eq("ordem_producao_id", params.ordem_producao_id);
            if (params.data_inicio)
                query = query.gte("inicio", params.data_inicio);
            if (params.data_fim)
                query = query.lte("inicio", params.data_fim + "T23:59:59");
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const a of items) {
                        const etapaNome = a.producao_etapas?.nome ?? "—";
                        const operador = a.profiles?.full_name ?? "—";
                        lines.push(`- **${etapaNome}** — ${a.tempo_minutos} min — ${operador} — ${formatDate(a.inicio)}`);
                        if (a.observacoes)
                            lines.push(`  _${a.observacoes}_`);
                    }
                }
                text = lines.join("\n");
            }
