...
export function registerImpressoraTools(server) {
    // ─── croma_listar_jobs_impressora ─────────────────────────────────────────
    server.registerTool("croma_listar_jobs_impressora", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                ...
                .select(`id, documento, estado, area_m2, substrato, modo_impressao, data_impressao,
             ...
             pedidos(numero, status)`, { count: "exact" })
                .order("data_impressao", { ascending: false });
            if (params.data_inicio)
                query = query.gte("data_impressao", `${params.data_inicio}T00:00:00`);
            if (params.data_fim)
                query = query.lte("data_impressao", `${params.data_fim}T23:59:59`);
            if (params.estado)
                query = query.eq("estado", params.estado);
            if (params.cliente)
                query = query.ilike("cliente_extraido", `%${params.cliente}%`);
            if (params.pedido_id)
                query = query.eq("pedido_id", params.pedido_id);
            if (params.apenas_sem_vinculo)
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const j of items) {
                        ...
                        const clienteNome = j.clientes?.nome_fantasia
                            ?? j.clientes?.razao_social
                            ?? j.cliente_extraido
                            ?? "—";
                        const pedidoNum = j.pedidos?.numero;
                        const vinculo = pedidoNum ? `📋 Pedido ${pedidoNum}` : "⚠️ Sem vínculo";
                        const metodo = j.metodo_custeio === "LM_ancora" ? "LM âncora" : "média histórica";
                        const alertasArr = j.alertas ?? [];
    ...
    server.registerTool("croma_resumo_impressora", {
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            for (const j of impressos) {
                ...
                if (!porCliente[c])
                    porCliente[c] = { m2: 0, custo: 0, jobs: 0, custoTinta: 0, custoSub: 0, custoMaq: 0 };
            ...
            for (const j of impressos) {
                ...
                if (!porDia[dia])
                    porDia[dia] = { m2: 0, jobs: 0, custo: 0 };
            ...
            for (const j of jobs) {
                const al = j.alertas ?? [];
                for (const a of al)
                    todosAlertas.push(`[${j.cliente_extraido}] ${a}`);
            }
            const resumo = {
                periodo: { inicio, fim },
                totais: {
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (todosAlertas.length > 0) {
                    ...
                    for (const a of todosAlertas.slice(0, 10))
                        lines.push(`- ⚠️ ${a}`);
                    if (todosAlertas.length > 10)
                        lines.push(`_...e mais ${todosAlertas.length - 10} alertas._`);
                }
                text = lines.join("\n");
            }
            else {
    ...
    server.registerTool("croma_vincular_job_impressora", {
...
Args:
  ...
  - ordem_producao_id (string, opcional): ID da OP`,
        ...
        try {
            ...
            const updateData = {};
            if (params.pedido_id)
                updateData.pedido_id = params.pedido_id;
            if (params.cliente_id)
                updateData.cliente_id = params.cliente_id;
            if (params.ordem_producao_id)
                updateData.ordem_producao_id = params.ordem_producao_id;
            if (Object.keys(updateData).length === 0) {
                return {
                    content: [{ type: "text", text: "Erro: Informe pelo menos um vínculo (pedido_id, cliente_id ou ordem_producao_id)." }],
                };
            }
            const { data, error } = await sb
                .from("impressora_jobs")
    ...
    server.registerTool("croma_registrar_jobs_impressora", {
...
Args:
  - jobs (array): Lista de jobs com campos obrigatórios e opcionais`,
    ...
    server.registerTool("croma_custo_real_pedido", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            if (params.pedido_id)
                query = query.eq("pedido_id", params.pedido_id);
            if (params.pedido_numero)
                query = query.ilike("pedido_numero", `%${params.pedido_numero}%`);
    ...
    server.registerTool("croma_mapear_substrato", {
  ...
  - material_id (string, para mapear): ID do material no catálogo`,
        ...
        try {
            ...
            if (!params.nome_ews || !params.material_id) {
                return {
                    content: [{ type: "text", text: "Erro: Para mapear, informe nome_ews e material_id." }],
                };
            }
            const sb = getUserClient();
            const updateData = { material_id: params.material_id };
            if (params.custo_m2_override)
                updateData.custo_m2_override = params.custo_m2_override;
            const { data, error } = await sb
                .from("impressora_substrato_map")
                .update(updateData)
...
//# sourceMappingURL=impressora.js.map