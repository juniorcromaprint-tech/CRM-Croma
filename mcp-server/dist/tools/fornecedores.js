...
import { formatBRL, formatDate, formatCNPJ, formatPhone, } from "../utils/formatting.js";
export function registerFornecedoresTools(server) {
    // ─── croma_listar_fornecedores ─────────────────────────────────────────────
    server.registerTool("croma_listar_fornecedores", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Formato da resposta (padrão: markdown)`,
        ...
        try {
            ...
            let query = sb
                .from("fornecedores")
                .select("id, razao_social, nome_fantasia, cnpj, telefone, email, contato_nome, categorias, lead_time_dias, condicao_pagamento, ativo", { count: "exact" });
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const f of items) {
                        ...
                        lines.push(`- **ID**: \`${f.id}\``);
                        if (f.cnpj)
                            lines.push(`- **CNPJ**: ${formatCNPJ(f.cnpj)}`);
                        if (f.telefone)
                            lines.push(`- **Telefone**: ${formatPhone(f.telefone)}`);
                        if (f.email)
                            lines.push(`- **Email**: ${f.email}`);
                        if (f.contato_nome)
                            lines.push(`- **Contato**: ${f.contato_nome}`);
                        if (f.categorias?.length)
                            lines.push(`- **Categorias**: ${f.categorias.join(", ")}`);
                        if (f.lead_time_dias)
                            lines.push(`- **Lead time**: ${f.lead_time_dias} dias`);
                        if (f.condicao_pagamento)
                            lines.push(`- **Condição pgto**: ${f.condicao_pagamento}`);
    ...
    server.registerTool("croma_detalhe_fornecedor", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Formato da resposta (padrão: markdown)`,
        ...
        try {
            ...
            if (fornResult.error)
                return errorResult(fornResult.error);
            if (!fornResult.data) {
                return { content: [{ type: "text", text: `Fornecedor não encontrado: ${params.fornecedor_id}` }] };
            }
            const f = fornResult.data;
            const contas = contasResult.data ?? [];
            const totalCompras = (resumoResult.data ?? []).reduce((sum, c) => sum + (Number(c.valor_original) || 0), 0);
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                const lines = [
                    ...
                    f.categorias?.length ? `- **Categorias**: ${f.categorias.join(", ")}` : "",
    ...
    server.registerTool("croma_cadastrar_fornecedor", {
...
Retorna: dados do fornecedor criado com ID gerado`,
        inputSchema: z.object({
            ...
            categorias: z.preprocess((val) => typeof val === 'string' ? JSON.parse(val) : val, z.array(z.string()).optional()).describe("Categorias de produtos fornecidos"),
    ...
    server.registerTool("croma_atualizar_fornecedor", {
...
Args:
  ...
  - observacoes (string, opcional): Novas observações`,
        inputSchema: z.object({
            ...
            contato_nome: z.string().max(100).optional(),
            categorias: z.preprocess((val) => typeof val === 'string' ? JSON.parse(val) : val, z.array(z.string()).optional()),
            lead_time_dias: z.coerce.number().int().min(0).optional(),
            condicao_pagamento: z.string().max(100).optional(),
        ...
        try {
            ...
            if (error)
                return errorResult(error);
            if (!data)
                return { content: [{ type: "text", text: `Fornecedor não encontrado: ${fornecedor_id}` }] };
            return {
                content: [{
                        type: "text",
    ...
    server.registerTool("croma_historico_compras_fornecedor", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Formato da resposta (padrão: markdown)`,
        ...
        try {
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, contas.length + precos.length) }],
                structuredContent: fullData,
            };
        }
        catch (error) {
