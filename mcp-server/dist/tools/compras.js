...
export function registerComprasTools(server) {
    // ─── croma_listar_compras ──────────────────────────────────────────────────
    server.registerTool("croma_listar_compras", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            let query = sb
                .from("pedidos_compra")
                .select("id, numero, status, valor_total, previsao_entrega, observacoes, created_at, fornecedores(razao_social, nome_fantasia)", { count: "exact" })
                .is("excluido_em", null);
            if (params.filtro_status)
                query = query.eq("status", params.filtro_status);
            if (params.filtro_fornecedor)
                query = query.eq("fornecedor_id", params.filtro_fornecedor);
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (items.length === 0) {
                    ...
                    for (const pc of items) {
                        ...
                        if (pc.previsao_entrega)
                            lines.push(`- **Previsão entrega**: ${formatDate(pc.previsao_entrega)}`);
                        lines.push(`- **Criado em**: ${formatDate(pc.created_at)}`);
                        if (pc.observacoes)
                            lines.push(`- **Obs**: ${pc.observacoes}`);
                        lines.push("");
                    }
                }
                text = lines.join("\n");
    ...
    server.registerTool("croma_detalhe_compra", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Padrão markdown`,
        ...
        try {
            ...
            if (pcResult.error)
                return errorResult(pcResult.error);
            if (!pcResult.data) {
                return { content: [{ type: "text", text: `Pedido de compra não encontrado: ${params.pedido_compra_id}` }] };
            }
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, itens.length) }],
    ...
    // ─── croma_criar_compra ────────────────────────────────────────────────────
    server.registerTool("croma_criar_compra", {
...
Retorna: ID do pedido + resumo dos itens`,
        ...
        try {
            ...
            const valorTotal = params.itens.reduce((sum, item) => sum + item.quantidade * item.preco_unitario, 0);
            // Criar cabeçalho
    ...
    server.registerTool("croma_atualizar_status_compra", {
...
Args:
  ...
  - observacoes (string, opcional): Motivo ou observação`,
        ...
        try {
            ...
            const updateData = { status: params.novo_status, updated_at: new Date().toISOString() };
    ...
    server.registerTool("croma_registrar_recebimento", {
