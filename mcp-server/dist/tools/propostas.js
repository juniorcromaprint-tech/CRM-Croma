...
export function registerPropostasTools(server) {
    // ─── croma_listar_propostas ──────────────────────────────────────────────
    server.registerTool("croma_listar_propostas", {
...
Args:
  ...
  - response_format ('markdown'|'json'): Formato (padrão: markdown)`,
        ...
        try {
            ...
            let query = sb
                .from("propostas")
                .select(`id, numero, status, titulo, total, subtotal, desconto_percentual,
            ...
            if (params.valor_min !== undefined)
                query = query.gte("total", params.valor_min);
            if (params.valor_max !== undefined)
                query = query.lte("total", params.valor_max);
            query = query
                ...
                if (items.length === 0) {
                    ...
                    for (const p of items) {
                        ...
                        if (p.desconto_percentual)
                            lines.push(`- **Desconto**: ${p.desconto_percentual}%`);
                        if (p.validade_dias)
                            lines.push(`- **Validade**: ${p.validade_dias} dias`);
    ...
    server.registerTool("croma_detalhe_proposta", {
  ...
  - response_format ('markdown'|'json'): Formato (padrão: markdown)`,
        ...
        try {
            ...
            if (propostaResult.error)
                return errorResult(propostaResult.error);
            ...
            if (params.response_format === ResponseFormat.MARKDOWN) {
                ...
                if (itens.length > 0) {
                    ...
                    for (const item of itens) {
                        ...
                        if (item.material_descricao)
                            lines.push(`- **Material**: ${item.material_descricao}`);
                    }
                }
                text = lines.join("\n");
            }
            ...
            return {
                content: [{ type: "text", text: truncateIfNeeded(text, itens.length + 1) }],
    ...
    server.registerTool("croma_criar_proposta", {
  ...
  - itens (array, obrigatório): Lista de itens da proposta
    ...
    - material_descricao (string, opcional): Descrição do material`,
        inputSchema: z.object({
            ...
            itens: z.preprocess((val) => typeof val === 'string' ? JSON.parse(val) : val, z.array(z.object({
        ...
        try {
            ...
            // Calcula totais
            const subtotal = params.itens.reduce((sum, item) => sum + item.quantidade * item.valor_unitario, 0);
            ...
            const { data: proposta, error: propostaError } = await sb
                ...
                .single();
            if (propostaError)
                return errorResult(propostaError);
            // Cria os itens
    ...
    server.registerTool("croma_atualizar_status_proposta", {
...
Args:
  ...
  - motivo (string, opcional): Motivo da mudança (obrigatório ao recusar)`,
        ...
        try {
            ...
            if (params.motivo)
                updateData.observacoes = params.motivo;
...
Obs: Informe proposta_id OU proposta_numero. A proposta é marcada como "enviada" automaticamente.`,
        ...
        try {
            if (!params.proposta_id && !params.proposta_numero) {
                return {
                    content: [{ type: "text", text: "❌ Informe proposta_id (UUID) ou proposta_numero (ex: PROP-2026-0007)." }],
                };
            }
            const sb = getAdminClient();
            ...
            if (error || !proposta) {
                ...
                return { content: [{ type: "text", text: `❌ Proposta não encontrada: ${ref}` }] };
            }
