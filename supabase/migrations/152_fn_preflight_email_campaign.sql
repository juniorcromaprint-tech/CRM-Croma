-- Migration 152: pre-flight de campanha de email
--
-- Roda 10 checks ANTES de disparar uma campanha e retorna PASS/FAIL/WARN por check
-- + lista de leads bloqueados como exemplos.
--
-- Uso: SELECT * FROM public.fn_preflight_email_campaign(ARRAY[uuid1,uuid2,...], '4259f1dd-...');
...
BEGIN
  ...
  RETURN QUERY SELECT
    ...
    CASE WHEN v_template.id IS NULL THEN 'Template nao encontrado'
         WHEN NOT v_template.ativo THEN 'Template existe mas esta inativo'
         ELSE 'OK' END,
    NULL::text[];
  ...
  SELECT
    ...
    ARRAY_AGG(pt.p)
  FROM placeholders_no_template pt
  WHERE pt.p NOT IN (SELECT p FROM suportados);

  RETURN QUERY
  SELECT 'leads_com_nao_incluir'::text,
    CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*)::int,
    CASE WHEN COUNT(*) = 0 THEN 'OK' ELSE COUNT(*) || ' leads marcados [NAO INCLUIR]' END,
    (ARRAY_AGG(empresa ORDER BY empresa))[1:5]
  FROM public.leads WHERE id = ANY(p_lead_ids)
    AND (observacoes ILIKE '%NAO INCLUIR%' OR observacoes ILIKE '%NÃO INCLUIR%');

  RETURN QUERY
  SELECT 'leads_sem_email_valido'::text,
    CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*)::int,
    CASE WHEN COUNT(*) = 0 THEN 'OK' ELSE COUNT(*) || ' leads sem email valido' END,
    (ARRAY_AGG(empresa ORDER BY empresa))[1:5]
  FROM public.leads WHERE id = ANY(p_lead_ids)
    AND (contato_email IS NULL OR contato_email !~* '^[^@\s]+@[^@\s]+\.[^@\s]+$');
  ...
  RETURN QUERY
  WITH dup AS (SELECT contato_email, COUNT(*) c FROM public.leads
    WHERE id = ANY(p_lead_ids) AND contato_email IS NOT NULL
  ...
  RETURN QUERY
  SELECT 'leads_em_conversa_ativa'::text,
    CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*)::int,
  ...
  RETURN QUERY
  SELECT 'leads_excluidos'::text,
    CASE WHEN COUNT(*) = 0 THEN 'PASS' ELSE 'FAIL' END, COUNT(*)::int,
    CASE WHEN COUNT(*) = 0 THEN 'OK' ELSE COUNT(*) || ' leads excluidos' END, NULL::text[]
  FROM public.leads WHERE id = ANY(p_lead_ids) AND excluido_em IS NOT NULL;

  RETURN QUERY
  SELECT 'respeita_limite_diario'::text,
...
COMMENT ON FUNCTION public.fn_preflight_email_campaign(uuid[], uuid) IS
'10 checks PASS/FAIL/WARN antes de disparar campanha de email. Usar antes de qualquer fn_disparar_abertura_em_massa.';
