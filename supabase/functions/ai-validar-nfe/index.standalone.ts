// ai-validar-nfe — STANDALONE (sem imports relativos)
// Deploy: npx supabase@latest functions deploy ai-validar-nfe --project-ref djwjmfgplnqyffdcgdaw
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!pedido_id && !documento_id) {
      return jsonResponse({ error: 'pedido_id ou documento_id é obrigatório' }, 400);
    }
    ...
    if (documento_id && !pedidoId) {
      const { data: doc } = await supabase.from('fiscal_documentos').select('pedido_id').eq('id', documento_id).single();
      pedidoId = doc?.pedido_id;
    }
    if (!pedidoId) return jsonResponse({ error: 'Pedido não encontrado' }, 404);

    const { data: pedido } = await supabase
      .from('pedidos')
      .select(`
        id, numero, valor_total, status,
        clientes(razao_social, nome_fantasia, cnpj_cpf, inscricao_estadual, endereco, cidade, estado, cep, email, telefone)
      `)
      .eq('id', pedidoId)
      .single();
    ...
    const { data: fiscalConfig } = await supabase.from('fiscal_configuracao').select('*').limit(1).single();
    ...
    if (!cliente?.endereco) erros.push({ campo: 'cliente.endereco', valor_atual: null, valor_sugerido: null, motivo: 'Endereço do cliente é obrigatório' });
    if (!cliente?.cidade) erros.push({ campo: 'cliente.cidade', valor_atual: null, valor_sugerido: null, motivo: 'Cidade do cliente é obrigatória' });
    if (!cliente?.estado) {
    ...
    if (!cliente?.cep) avisos.push({ campo: 'cliente.cep', valor_atual: null, valor_sugerido: null, motivo: 'CEP não preenchido — recomendado para NF-e' });
    ...
    if (apiKey && itens && itens.length > 0 && erros.length === 0) {
      try {
        const itemsDesc = itens.map(i => `${i.descricao} | NCM: ${i.ncm ?? 'vazio'} | CFOP: ${i.cfop ?? 'vazio'} | Valor: ${i.valor_total}`).join('\n');
...
Se tudo estiver OK, responda: {"sugestoes": []}`;
        ...
        if (aiRes.ok) {
          ...
          if (match) {
            ...
            for (const s of parsed.sugestoes ?? []) {
              sugestoes.push({ campo: `${s.item} (${s.campo})`, valor_atual: s.atual, valor_sugerido: s.sugerido, motivo: s.motivo });
            }
          }
        }
    ...
    return jsonResponse({ valido, erros, avisos, sugestoes }, 200);
